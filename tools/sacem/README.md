...
- Profondeur : `build_ext.py` envoie tout le bâtiment (murs, faces intérieures, encadrement de la porte, haut des bibliothèques) dans la peinture `above` (dessinée par-dessus le woka), sauf le sol, le parvis et la bande de 40 px des éléments posés juste au-dessus du sol (pied du mur du fond, bas des bibliothèques) qui restent dans `walls`. Le woka est donc masqué quand il longe le bas de la pièce ou passe derrière le bâtiment (sur la face intérieure du mur du fond, libre dans ses collisions), et reste devant le mur du fond quand il est sur le sol.
