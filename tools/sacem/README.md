...
- Profondeur : `build_ext.py` classe chaque pixel de mur d'après la direction du sol du hall le plus proche : sol au-dessus → mur avant → peinture `above` (le woka passe derrière) ; sol à côté → mur latéral → `walls` (le woka qui le longe reste devant, pas de demi-masquage) ; sol au-dessous → mur du fond : `walls` à moins de 40 px du sol (pied du mur, bibliothèques), `above` au-delà (face intérieure vue depuis l'extérieur, le woka y est caché). La décision est prise par tuile (classe majoritaire) pour éviter les demi-masquages dans une même case. Limite inhérente aux calques : dans une case mêlant sol et mur avant, le woka est coupé par l'arête du mur (effet « derrière le parapet »).
